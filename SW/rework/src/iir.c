#include "../inc/iir.h"
#include "../inc/conv.h"

/**************************************
 * IIR filtar prvog reda
 *
 * input - ulazni odbirak
 * coefficients - koeficijenti [a0 a1 b0 b1]
 * z_x - memorija za ulazne odbirke (niz duzine 2)
 * z_y - memorija za izlazne odbirke (niz duzine 2)
 *
 * povratna vrednost - izlazni odbirak
 *
 **************************************/

int16_t first_order_IIR(int16_t input, int16_t* coefficients, int16_t* z_x, int16_t* z_y) {
	int32_t temp;

	z_x[0] = input; /* Copy input to x[0] */

	temp = ((int32_t) coefficients[0] * z_x[0]); /* B0 * x(n)     */
	temp += ((int32_t) coefficients[1] * z_x[1]); /* B1 * x(n-1) */
	temp -= ((int32_t) coefficients[3] * z_y[1]); /* A1 * y(n-1) */

	/* Divide temp by coefficients[A0] to remove fractional part */
	temp >>= 15;

	z_y[0] = (int16_t) (temp);

	/* Range limit temp between maximum and minimum */

	if (temp > 32767) {
		temp = 32767;
	} else if (temp < -32767) {
		temp = -32767;
	}

	/* Shuffle values along one place for next time */

	z_y[1] = z_y[0]; /* y(n-1) = y(n)   */
	z_x[1] = z_x[0]; /* x(n-1) = x(n)   */

	return ((int16_t) temp);
}

/**************************************
 * IIR filtar drugog reda
 *
 * input - ulazni odbirak
 * coefficients - koeficijenti [a0 a1/2 a2 b0 b1/2 b2]
 * z_x - memorija za ulazne odbirke (niz duzine 3)
 * z_y - memorija za izlazne odbirke (niz duzine 3)
 *
 * povratna vrednost - izlazni odbirak
 *
 * NAPOMENA: Koeficijenti b1 i a1 se nalaze u opsegu [-2, 2),
 * stoga se prosledjuje polovina njihove vrednosti kako bi
 * mogli biti prezentovani u 1.31 prezentaciji brojeva u
 * nepokretnom zarezu
 *************************************/

int16_t second_order_IIR(int16_t input, int16_t* coefficients, int16_t* x_history, int16_t* y_history) {
    int32_t accum = 0;

    accum = signed_multipy_accumulate(accum, coefficients[0], input);        /* A0 * x(n)     */
    accum = signed_multipy_accumulate(accum, coefficients[1], x_history[0]); /* A1/2 * x(n-1) */
    accum = signed_multipy_accumulate(accum, coefficients[1], x_history[0]); /* A1/2 * x(n-1) */
    accum = signed_multipy_accumulate(accum, coefficients[2], x_history[1]); /* A2 * x(n-2)   */
    accum = signed_multipy_subtract(accum, coefficients[4], y_history[0]); /* B1/2 * y(n-1) */
    accum = signed_multipy_subtract(accum, coefficients[4], y_history[0]); /* B1/2 * y(n-1) */
    accum = signed_multipy_subtract(accum, coefficients[5], y_history[1]); /* B2 * y(n-2)   */

    accum >>= 16;

    y_history[1] = y_history[0];    /* y(n-2) = y(n-1) */
    y_history[0] = (int16_t) (accum); /* y(n-1) = y(n)   */
    x_history [1] = x_history [0];  /* x(n-2) = x(n-1) */
    x_history [0] = input;          /* x(n-1) = x(n)   */

    return ((int16_t) accum);
}



int16_t fourth_order_IIR(int16_t input, int16_t coefficients[][6], int16_t x_history[][3],
		int16_t y_history[][3]) {
	uint32_t accum;
	int16_t temp;
	uint16_t stage;

    temp = input; /* Copy input to temp */

	for ( stage = 0 ; stage < 2 ; stage++) /* for two stages */
	{
		accum = 0;
		accum = signed_multipy_accumulate(accum, coefficients[stage][0], temp);         /* A0 * x(n)     */
		accum = signed_multipy_accumulate(accum, coefficients[stage][1], x_history[stage][0]); /* A1/2 * x(n-1) */
		accum = signed_multipy_accumulate(accum, coefficients[stage][1], x_history[stage][0]); /* A1/2 * x(n-1) */
		accum = signed_multipy_accumulate(accum, coefficients[stage][2], x_history[stage][1]); /* A2 * x(n-2)   */
		accum = signed_multipy_subtract(accum, coefficients[stage][4], y_history[stage][0]); /* B1/2 * y(n-1) */
		accum = signed_multipy_subtract(accum, coefficients[stage][4], y_history[stage][0]); /* B1/2 * y(n-1) */
		accum = signed_multipy_subtract(accum, coefficients[stage][5], y_history[stage][1]); /* B2 * y(n-2)   */

		accum >>= 16;

		/* Shuffle values along one place for next time */
		y_history[stage][1] = y_history[stage][0];    /* y(n-2) = y(n-1) */
		y_history[stage][0] = (int16_t) (accum); /* y(n-1) = y(n)   */
		x_history[stage][1] = x_history[stage][0];  /* x(n-2) = x(n-1) */
		x_history[stage][0] = temp;          /* x(n-1) = x(n)   */

		/* temp is used as input next time through */
		temp = (int16_t)accum;
	}
	/* End loop */

	return (int16_t)accum;
}


int16_t Nth_order_IIR(int16_t input, int16_t coefficients[][6], int16_t z_x[][3],
		int16_t z_y[][3], int16_t order) {

	/* Copy input to temp */

	/* for N stages */

		/* Copy input to x[stages][0] */

		/* B0 * x(n)     */

		/* B1/2 * x(n-1) */

		/* B1/2 * x(n-1) */

		/* B2 * x(n-2)   */

		/* A1/2 * y(n-1) */

		/* A1/2 * y(n-1) */

		/* A2 * y(n-2)   */

		/* Divide temp by 2^15 to remove fractional part */

		/* Range limit temp between maximum and minimum */

		/* Shuffle values along one place for next time */

		/* y(n-2) = y(n-1) */
		/* y(n-1) = y(n)   */

		/* x(n-2) = x(n-1) */
		/* x(n-1) = x(n)   */

		/* temp is used as input next time through */
	
	/* End loop */

	return 0;
}

int16_t halfband(int16_t input, int16_t* z_x, int16_t* z_y, int16_t lowpass)
{
     return 0;
}


float second_order_IIR_Direct_Form_II_f(float input, float* coefficients, float* history) {
	float accum = 0;
	float temp = 0;

    accum = input;
    accum -= coefficients[4]*history[0];
    accum -= coefficients[5]*history[1];

    temp = accum;

    accum = coefficients[0] * accum;
    accum += coefficients[1] * history[0];
    accum += coefficients[2] * history[1];

    history[1] = history[0];
    history[0] = temp;

    return accum;
}

